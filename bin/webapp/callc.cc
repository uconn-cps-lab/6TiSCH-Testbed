...
#define KEY_LENGTH  (4*(AES_MAXROUNDS + 1))
...
extern "C" void rijndaelEncrypt(const uint32_t rk[/*4*(Nr + 1)*/], int Nr, const uint8_t pt[16],  uint8_t ct[16]);
extern "C" int rijndaelKeySetupEnc(uint32_t rk[/*4*(Nr + 1)*/], const uint8_t cipherKey[], int keyBits);

void callc_rijndaelKeySetupEnc(const FunctionCallbackInfo<Value>& args)
{
  Isolate* isolate = args.GetIsolate();
  Local<Array> cipherKey_in = Local<Array>::Cast(args[0]);
  int keyBits_in = args[1]->NumberValue();
  Local<Array> rk_out = Array::New(isolate);
  Local<Object> ret_obj = Object::New(isolate);

  uint32_t rk[KEY_LENGTH];
  uint8_t cipherKey[32];
  int retv;

  for (int i = 0; i < keyBits_in/8; i++)
  {
     cipherKey[i] = cipherKey_in->Get(i)->NumberValue();
  }

  retv = rijndaelKeySetupEnc(rk, cipherKey, keyBits_in);

  if (retv <= 10)
  {
     for (int i = 0; i < 4*(retv+1); i++)
     {
        rk_out->Set(i, Number::New(isolate, rk[i]));
     }
  }

  ret_obj->Set(String::NewFromUtf8(isolate, "ret"), Number::New(isolate, retv));
  ret_obj->Set(String::NewFromUtf8(isolate, "rk"), rk_out);

  args.GetReturnValue().Set(ret_obj);
}

void callc_rijndaelEncrypt(const FunctionCallbackInfo<Value>& args) 
{
  Isolate* isolate = args.GetIsolate();
  Local<Array> rk_in = Local<Array>::Cast(args[0]);
  int Nr = args[1]->NumberValue();
  Local<Array> pt_in = Local<Array>::Cast(args[2]);
  Local<Array> ct_out = Array::New(isolate);
  Local<Object> ret_obj = Object::New(isolate);
  ...
  {
     rk[i] = rk_in->Get(i)->NumberValue();
  }
  
  for (int i = 0; i < 16; i++)
  {
     pt[i] = pt_in->Get(i)->NumberValue();
  }
  ...
  for (int i = 0; i < 16; i++)
  {
     ct_out->Set(i, Number::New(isolate, ct[i]));
  }
  
  ret_obj->Set(String::NewFromUtf8(isolate, "ct"), ct_out);
...
void init(Local<Object> exports) { 
  NODE_SET_METHOD(exports, "rijndaelKeySetupEnc", callc_rijndaelKeySetupEnc);
  NODE_SET_METHOD(exports, "rijndaelEncrypt", callc_rijndaelEncrypt);
