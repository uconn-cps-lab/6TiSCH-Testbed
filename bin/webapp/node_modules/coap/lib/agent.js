...
var bl              = require('bl')
  ...
  , Dtls            = require('dtls')     //fengmo DTLS
  , dtls_config     = require('dtls/dtls_config.js') //fengmo DTLS
  , dtls_handle_message = require('dtls/lib/dtls_core').dtls_handle_message  //fengmo DTLS
...
Agent.prototype._init = function initSock() {
  if (this._sock)
   ...
   this._sock = dgram.createSocket(this._opts.type, function(msg, rsinfo) 
   {
      ...
      var decmsg = [];
      that.dtls.decrypt(rsinfo, msg, decmsg);   //fengmo DTLS
      
      if (decmsg.length > 0)
      {
         msg = Buffer.from(decmsg);
         ...
         catch(err) 
         {
            ...
            that.dtls.encrypt(message, rsinfo, function (encmsg, rsinfo)
            {
               var buf = Buffer.from(encmsg);
               that._sock.send(buf, 0, buf.length, rsinfo.port, rsinfo.address);
            });  //fengmo DTLS
            return
         }
   ...
   this.dtls = new Dtls(dtls_config.DTLS_CLIENT, {type:'udp6'}); //fengmo DTLS  create a new peer as a server (we are the client)
}
...
Agent.prototype._doClose = function() {
  if (this._msgIdToReq)
  {
     ...
     {
        if (this._msgIdToReq[k].sender && !this._msgIdToReq[k].sender.deleted)
        {
           if (this._msgIdToReq[k].sender.reset)
           {
              this._msgIdToReq[k].sender.reset();
           }
           
           this._msgIdToReq[k].sender.deleted = true;
        }
     }
  }

  if (this._sock && !this._sock.deleted)
  {
     this._sock.close();
     this._sock.deleted = true;
  }
  
  if (this.dtls && !this.dtls.deleted)
  {
     this.dtls.close();  //fengmo DTLS
     this.dtls.deleted = true;
  }
}
...
Agent.prototype._handle = function handle(msg, rsinfo, outSocket) {
  ...
  if (!req) {
    ...
    if (!req) {
      ...
      this._msgInFlight++
      this.dtls.encrypt(buf, rsinfo, function (encmsg, rsinfo)
      {
         var buf = Buffer.from(encmsg);
         that._sock.send(buf, 0, buf.length, rsinfo.port, rsinfo.address, ackSent);
      });  //fengmo DTLS
...
Agent.prototype.request = function request(url) {
  ...
  req.freeCoapClientMemory = freeCoapClientMemory;
  req.sender = new RetrySend(this._sock, url.port, url.hostname || url.host, this.dtls); //fengmo
...
function freeCoapClientMemory()
{
   //console.log("freeCoapClientMemory() enter");
   if (this.sender && !this.sender.deleted)
   {
      //console.log("freeCoapClientMemory() 1");
      this.sender.removeAllListeners();
      this.sender.reset();
      this.sender.deleted = true;
      //console.log("freeCoapClientMemory() 2");
   }
   
   if (this.url && !this.url.deleted)
   {
      if(this.url.agent)
      {
         //console.log("freeCoapClientMemory() 3");
         this.url.agent._doClose(); 
         //console.log("freeCoapClientMemory() 4");
      }
      this.url.deleted = true;
   }
   
   //console.log("freeCoapClientMemory() 5");
   this.statusCode = null;

   //console.log("freeCoapClientMemory() exit");
}
