 ...
 *
 * PD - This is an example coap_server
 * 		The server implemented in main.c is roughly based on this
 */
...
void hnd_get_time(coap_context_t *ctx, struct coap_resource_t *resource,
	...
	if (my_clock_base) {
		...
		if (request != NULL
				&& (option = coap_check_option(request, COAP_OPTION_URI_QUERY,
						&opt_iter))
...
void init_resources(coap_context_t *ctx) {
	...
	coap_add_attr(r, (unsigned char *) "rt", 2, (unsigned char *) "\"Ticks\"",
			7, 0);
	r->observable = 1;
