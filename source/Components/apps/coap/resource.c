...
int match(const str *text, const str *pattern, int match_prefix,
		int match_substring) {
	assert(text);
	assert(pattern);
...
#define MATCH_SUBSTRING 0x04
	static const str _rt_attributes[] = { { 2, (unsigned char *) "rt" }, { 2,
			(unsigned char *) "if" }, { 3, (unsigned char *) "rel" },
			{ 0, NULL } };
	...
	if (query_filter) {
		...
		if (resource_param.length < COAP_OPT_LENGTH(query_filter)) {
			...
			query_pattern.s = COAP_OPT_VALUE(query_filter)
					+ resource_param.length + 1;

			assert(
					(resource_param.length + 1) <= COAP_OPT_LENGTH(query_filter));
			query_pattern.length = COAP_OPT_LENGTH(query_filter)
					- (resource_param.length + 1);
	...
	for (i = 0; i < resource_storage.num; ++i, ++r) {
		...
		if (resource_param.length) { /* there is a query filter */
			...
			if (flags & MATCH_URI) { /* match resource URI */
				...
				coap_attr_t *attr;
				str unquoted_val;
				attr = coap_find_attr(r, resource_param.s,
						resource_param.length);
				if (!attr)
					continue;
...
coap_attr_t *
coap_find_attr(coap_resource_t *resource, const unsigned char *name,
		size_t nlen) {
	coap_attr_t *attr;
...
#endif //(WITH_POSIX) || defined(WITH_LWIP)
...
coap_print_status_t coap_print_link(const coap_resource_t *resource,
	...
	*len = 0;
	...
	COPY_COND_WITH_OFFSET(p, bufend, *offset, resource->uri.s,
			resource->uri.length, *len);
	...
	for (attr = list_head(resource->link_attr); attr;
		...
		COPY_COND_WITH_OFFSET(p, bufend, *offset, attr->name.s,
				attr->name.length, *len);

		if (attr->value.s) {
			PRINT_COND_WITH_OFFSET(p, bufend, *offset, '=', *len);

			COPY_COND_WITH_OFFSET(p, bufend, *offset, attr->value.s,
		...
		if (coap_address_equals(&s->subscriber, peer)
				&& (!token
						|| (token->length == s->token_length
								&& memcmp(token->s, s->token, token->length)
										== 0)))
			return s;
	}
...
coap_subscription_t *
coap_add_observer(coap_resource_t *resource, const coap_address_t *observer,
		const str *token) {
	coap_subscription_t *s;
...
static void coap_notify_observers(coap_context_t *context, coap_resource_t *r) {
	...
	if (r->observable && (r->dirty || r->partiallydirty)) {
		...
		assert(h);
		/* we do not allow subscriptions if no
		...
		for (obs = list_head(r->subscribers); obs; obs = list_item_next(obs)) {
			...
			if (obs->non && obs->non_cnt < coapConfig.coap_obs_max_non) {
...
static void coap_remove_failed_observers(coap_context_t *context,
		coap_resource_t *resource, const coap_address_t *peer, const str *token) {
	coap_subscription_t *obs;
...
#define INET6_ADDRSTRLEN 40
					...
					if (coap_print_addr(&obs->subscriber, addr,
							INET6_ADDRSTRLEN + 8))
						debug("** removed observer %s\n", addr);
				}
#endif
				coap_cancel_all_messages(context, &obs->subscriber, obs->token,
						obs->token_length);
...
void coap_delete_all_observers(void) 
{
	...
	for (i = 0; i < resource_storage.num; ++i, ++r) 
	{
		if (resource_storage.count[i]) 
		{
			if (r->observable)
			{
				for (s = list_head(r->subscribers); s; s = list_item_next(s)) 
				{
					list_remove(r->subscribers, s);
					COAP_FREE_TYPE(subscription, s);
				}
