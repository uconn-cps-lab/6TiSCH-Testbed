...
extern struct memb resource_storage;
//Jiachen
extern int COAP_SENSOR_RESOURCE_DIRTY_FREQ;
...
#ifdef LINUX_GATEWAY
  tcpip_process_post_func(HANDLE_COAP_RETRANSMIT, NULL);  //JIRA49
#else
  TCPIP_cmdmsg_t  msg;
...
#if COAP_MESSAGING_MOD
#ifdef WITHOUT_OBSERVE
MEMB( node_storage, coap_queue_t, 1); // Try to save space, only need one PDU wide buffer for received PDUs
#else
MEMB( node_storage, coap_queue_t, COAP_PDU_MAXCNT); // If WITH_OBSERVE, need multiple wide PDU buffer
#endif/*WITHOUT_OBSERVE*/
#else
MEMB( node_storage, coap_queue_t, COAP_PDU_MAXCNT);
PROCESS(coap_retransmit_process, "message retransmit process");
#endif /* COAP_MESSAGING_MOD*/
...
static void received_package(void *arg, struct udp_pcb *upcb, struct pbuf *p, ip_addr_t *addr, u16_t port)
{
	...
	context->pending_address.addr = addr->addr; /* FIX ME: this has to become address-type independent, probably there'll be an lwip function for that */
...
#else /* not a GCC */
coap_print_status_t print_wellknown(coap_context_t *context, unsigned char *buf,
...
/**
 * 		Consider removing parameter for coap_new_context()
 *		Was previously used to create/bind udp_connection,
 *		But that now takes place in main.c tsch_uip_leaf_task
*/
coap_context_t *
...
#ifdef WITH_CONTIKI
#if ! COAP_MESSAGING_MOD
	c->conn = udp_new(NULL, 0, NULL, NULL); // done in main.c leaf task
	udp_bind(c->conn, listen_addr->port); // done in main.c leaf task

	process_start(&coap_retransmit_process, (char *) c);

	Change context in order to set the timer
	PROCESS_CONTEXT_BEGIN(&coap_retransmit_process);
#ifndef WITHOUT_OBSERVE
   etimer_set(&c->notify_timer,
              coapConfig.coap_resource_check_time * COAP_TICKS_PER_SECOND / COAP_SUBSCRIBE_FREQ); // etimers not used in Sysbios - Need to update if observe is included
#endif /* WITHOUT_OBSERVE */
	/* the retransmit timer must be initialized to some large value */
	etimer_set(&the_coap_context.retransmit_timer, 0xFFFF); // Only used in Contiki
	PROCESS_CONTEXT_END(&coap_retransmit_process); // Only used in Contiki
#else /* COAP_MESSAGING_MOD */
        
#if I3_MOTE_SPLIT_ARCH
        extern uint32_t coap_ObservePeriod;
        /* The period setting from MSP432 is in milisecond.*/
        etimer_set(&c->notify_timer, &coap_timer_handler_obj, coap_ObservePeriod * (COAP_TICKS_PER_SECOND/1000));
#else
        etimer_set(&c->notify_timer, &coap_timer_handler_obj, coapConfig.coap_resource_check_time * COAP_TICKS_PER_SECOND / COAP_SUBSCRIBE_FREQ);
#endif        
#endif /* COAP_MESSAGING_MOD */
	return c;
#endif /* WITH_CONTIKI */
...
void coap_transaction_id(const coap_address_t *peer, const coap_pdu_t *pdu,
	...
	/* FIX ME: with lwip, we can do better */
...
#ifdef WITH_CONTIKI
//toport must be in Host byte order
void uip_udp_packet_send_post(struct udp_simple_socket *s, void *data, int len,
                              const uip_ipaddr_t *toaddr, uint16_t toport);

/* releases space allocated by PDU if free_pdu is set */
coap_tid_t coap_send_impl(coap_context_t *context, const coap_address_t *dst,
		coap_pdu_t *pdu) {
	coap_tid_t id = COAP_INVALID_TID;
	...
	/* FIX ME: is there a way to check if send was successful? */
	...
	uip_ipaddr_copy(&(session.addr), &(dst->addr));
	session.port = UIP_NTOHS(dst->port);
	session.size = sizeof(session.addr) + sizeof(session.port);
	peer = dtlsGetPeer(dtls_context[0], &session, &peerBuf);
	if (peer != NULL && peer->state == DTLS_STATE_CONNECTED)
	{
	     dtls_write(dtls_context[0], &session, (uint8_t *)(pdu->hdr), pdu->length);
	}
	else
#endif
	{
        uip_udp_packet_send_post(&context->udp_socket, pdu->hdr, pdu->length, &dst->addr, UIP_NTOHS(dst->port));//port comes in Network order, change to Host order
	}
	coap_transaction_id(dst, pdu, &id);
...
coap_send_impl(coap_context_t *context,
	...
	/* FIX ME: we can't check this here with the existing infrastructure, but we
	...
	pbuf_header(p, -(ptrdiff_t)((uint8_t*)pdu - (uint8_t*)p->payload) - sizeof(coap_pdu_t)); /* FIX ME hack around udp_sendto not restoring; see http://lists.gnu.org/archive/html/lwip-users/2013-06/msg00008.html. for udp over ip over ethernet, this was -42; as we're doing ppp too, this has to be calculated generically */
...
coap_tid_t coap_send_confirmed(coap_context_t *context,
   ...
   node->timeout = coapConfig.coap_default_response_timeout * COAP_TICKS_PER_SECOND
         + (coapConfig.coap_default_response_timeout >> 1)
	...
	{ /* (re-)initialize retransmission timer */
		...
		assert(nextpdu);
		/* we have just inserted a node */

		/* must set timer within the context of the retransmit process */
#if COAP_MESSAGING_MOD
                if (etimer_expired(&context->retransmit_timer))
                {
                  etimer_set(&context->retransmit_timer, &coap_timer_handler_obj, nextpdu->t);
                }
#else
		PROCESS_CONTEXT_BEGIN(&coap_retransmit_process);
...
int coap_read(coap_context_t *ctx){
	...
	coap_hdr_t *pdu;
#if COAP_MESSAGING_MOD
	size_t bytes_read = 0; //ssize_t not defined in Sysbios
#else
	ssize_t bytes_read = -1;
#endif /* COAP_MESSAGING_MOD*/
	coap_address_t src, dst;
	coap_queue_t *node;
	char *uip_rx_appdata = &(appRxFifo[appRxFifo_ReadIdx].c[UIP_IPUDPH_LEN + UIP_LLH_LEN]);
	uint16_t uip_rx_app_datalen = appRxLenFifo[appRxFifo_ReadIdx];

#ifdef WITH_CONTIKI
	buf = uip_rx_appdata;  //fengmo debug node packet loss
#endif /* WITH_CONTIKI */
#ifdef WITH_LWIP
...
#ifdef WITH_CONTIKI
#if COAP_MESSAGING_MOD
		uip_ipaddr_copy(&src.addr, &APP_UIP_IP_BUF->srcipaddr);
		src.port = APP_UIP_UDP_BUF ->srcport;
		uip_ipaddr_copy(&dst.addr, &APP_UIP_IP_BUF->destipaddr);
		dst.port = APP_UIP_UDP_BUF->destport;
		bytes_read = uip_rx_app_datalen;//Get the app data length from the uIPv6 stack //coap_uip_len;  //fengmo debug node packet loss
	...
	if(uip_newdata()) {
		...
		((char *)uip_rx_appdata)[bytes_read] = 0;
...
#endif /* WITH_CONTIKI */
#ifdef WITH_LWIP
	/* FIX ME: use lwip address operation functions */
...
wellknown_response(coap_context_t *context, coap_pdu_t *request) {
	...
	assert(
			coap_encode_var_bytes(buf, COAP_MEDIATYPE_APPLICATION_LINK_FORMAT) == 1);
...
void handle_request(coap_context_t *context, coap_queue_t *node) {
	...
	if (!resource) {
		...
		case COAP_REQUEST_GET:
			if (is_wkc(key)) { /* GET request for .well-known/core */
				...
				response = coap_new_error_response(node->pdu,
						COAP_RESPONSE_CODE(404), opt_filter);
			}
			break;
		...
		default: /* any other request type */
			...
			if (!coap_is_mcast(&node->local))
				response = coap_new_error_response(node->pdu,
						COAP_RESPONSE_CODE(405), opt_filter);
		}

		if (response
				&& coap_send(context, &node->remote, response)
						== COAP_INVALID_TID) {
	...
	if (h) {
		...
		response = coap_pdu_init(
				node->pdu->hdr->type == COAP_MESSAGE_CON ?
						COAP_MESSAGE_ACK : COAP_MESSAGE_NON, 0,
				node->pdu->hdr->id, COAP_MAX_PDU_SIZE);
		...
		if (coap_add_token(response, node->pdu->hdr->token_length,
			...
			if (response->hdr->type != COAP_MESSAGE_NON
				...
				if (coap_send(context, &node->remote,
...
static inline void handle_response(coap_context_t *context, coap_queue_t *sent,
		coap_queue_t *rcvd) {

	/* Call application-specific reponse handler when available.  If
	 * not, we must acknowledge confirmable messages. */
	if (context->response_handler) {
		context->response_handler(context, &rcvd->remote,
				sent ? sent->pdu : NULL, rcvd->pdu, rcvd->id);
	} else {
...
void coap_dispatch(coap_context_t *context) {
   ...
   while (context->recvqueue) {
      ...
      if (rcvd->pdu->hdr->version != COAP_DEFAULT_VERSION) {
         debug(
               "dropped packet with unknown version %u\n", rcvd->pdu->hdr->version);
         goto cleanup;
      }
      ...
      case COAP_MESSAGE_ACK:
         ...
         /* FIX ME: if sent code was >= 64 the message might have been a
      ...
      case COAP_MESSAGE_CON: /* check for unknown critical options */
         if (coap_option_check_critical(context, rcvd->pdu, opt_filter)
            ...
            /* FIX ME: send response only if we have received a request. Otherwise,
             * send RST. */
...
#ifdef WITH_CONTIKI
#if COAP_MESSAGING_MOD
void coap_retransmit_observe_process() {
...
#ifndef HARP_MINIMAL
        extern coap_resource_t *sensor_resource;
        extern coap_resource_t *diagnosis_resource;
...
#ifdef WITH_LWIP
		/* FIX ME: retransmits that are not required any more due to incoming packages
